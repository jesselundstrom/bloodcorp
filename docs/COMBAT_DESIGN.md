...
- Arena grid is a configurable 13x9 logical grid with an ellipse mask, so valid playable tiles follow the arena footprint instead of a simple rectangle. The grid is a hidden tactical substrate; the player sees contextual movement reticles and tactical overlays instead of a full always-on lattice.
- Each battle selects one of three local arena layouts with named blockers, rough/raised/high terrain, ramp paths between elevation levels, Plasma Vents, player spawns, and enemy spawns.
- Player turns show clickable movement reticles for reachable empty spaces.
- Movement uses path-aware movement costs and step legality, so blockers, occupied units, invalid ellipse tiles, rough terrain, and ramp-gated elevation changes shape actual routes instead of only Manhattan distance.
- Blockers are impassable and stop Brutal Charge.
- Tile metadata stores grid coordinate, world position, walkable/blocked state, occupant id, terrain type, elevation, movement cost, cover type, hazard type, hazard state, and whether the tile is a ramp.
- Raised and high terrain are visually distinct. Higher attackers gain +1 attack total and +1 effective range against lower targets; attackers below their target take -1 to the attack total. Normal movement may change elevation only when stepping from or onto a ramp tile. Forced movement from higher to lower terrain deals fall damage.
...
- Units have `move_range`, `attack_range`, `has_moved`, `has_main_action`, and `has_bonus_action` fields, all reset each turn. Default `move_range` is derived from Speed: 1-3 => 2, 4-6 => 3, 7-9 => 4, 10+ => 5; explicit unit `move_range` values remain overrides for future equipment, cyberware, and skills.
...
- Battle presentation uses subtle contextual floor indicators: small cyan move reticles, red attackable-target rings/range outlines, white/yellow selected/active emphasis, tweened path movement for player/enemy moves plus shoves/charges, hit/miss/damage floaters, and downed/eliminated fade-outs. These are presentation hooks only; combat rules and save data are unchanged.
